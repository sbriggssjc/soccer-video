...
ffmpeg -y `
 ...
 -filter_complex "
   [1]scale=${FaceBox}:${FaceBox}:force_original_aspect_ratio=increase,crop=${FaceBox}:${FaceBox},zoompan=z='min(1.10,1.0+0.03*t)':d=${DurFrames}:s=${FaceBox}x${FaceBox},format=rgba,fade=t=in:st=0:d=${FadeInFace}:alpha=1,fade=t=out:st=$(($Dur-$FadeOutFace)):d=${FadeOutFace}:alpha=1[face];
