...
foreach ($r in $rows) {
 # common fields weve seen: clip (folder name), clip_id, index
 __TryAdd-IdxFromValue ([string]$r.clip) $r
 __TryAdd-IdxFromValue ([string]$r.clip_id) $r
 __TryAdd-IdxFromValue ([string]$r.index) $r
}

# If we still have branded candidates that wont match rows by idx, try folder names under cinematic.
...
try {
 ...
 # if rows didnt expose this idx yet, create a dummy row object we can attach branded_matches to
...
#  [string]$BrandedRoot = ".\out\portrait_reels\branded"
#  [string]$OutCsv = ".\atomic_clips_index.rebuilt.csv"
#  [string]$OutJson = ".\atomic_clips_index.rebuilt.json"
# )
# 
...
# [auto-comment extra param block] param([string]$name)
# 
#  $matchedName = $name
