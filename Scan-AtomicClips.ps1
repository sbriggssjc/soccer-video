...
foreach ($row in $rows) {
    ...
    # -- index-rescue patch --
    # If this branded file has no date (or didn’t match by tokens),
    # try to match by 3-digit clip index prefix (e.g., "002__...").
    if (-not $brand.Matched) {
        $bn = [System.IO.Path]::GetFileNameWithoutExtension($brand.Path)
        $brandIdx = ([regex]::Match($bn,'^(?<idx>\d{3})')).Groups['idx'].Value
        if ($brandIdx) {
...
.clip -and ([regex]::Match([CmdletBinding()]
param(
  [Parameter(Position=0)] [string]$RepoRoot      = ".",
  [Parameter(Position=1)] [string]$CinematicRoot = ".\out\autoframe_work\cinematic",
  [Parameter(Position=2)] [string]$BrandedRoot   = ".\out\portrait_reels\branded",
...
.clip,'^(?<idx>\d{3})')).Groups['idx'].Value -eq $brandIdx
