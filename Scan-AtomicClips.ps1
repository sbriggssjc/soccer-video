...
foreach ($r in $rows) {
  # common fields weÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢ve seen: clip (folder name), clip_id, index
  __TryAdd-IdxFromValue ([string]$r.clip)    $r
  __TryAdd-IdxFromValue ([string]$r.clip_id) $r
  __TryAdd-IdxFromValue ([string]$r.index)   $r
}

# If we still have branded candidates that wonÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢t match rows by idx, try folder names under cinematic.
  ...
  if (Test-Path -LiteralPath $cinRoot) {
    foreach ($d in (Get-ChildItem -LiteralPath $cinRoot -Directory -ErrorAction SilentlyContinue)) {
      ...
      if ($m.Success) {
        ...
        # if rows didnÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢t expose this idx yet, create a dummy row object we can attach branded_matches to
