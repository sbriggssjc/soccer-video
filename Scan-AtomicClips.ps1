...
# -- index-rescue patch --
# Match any still-unmatched branded files to clips by 3-digit prefix (e.g. "002__")
# Runs before branded-only rows are computed so counts reflect the rescue.
$__unmatched = $brandedInfo | Where-Object { -not $_.Matched }
foreach ($brand in $__unmatched) {
    $bn = [System.IO.Path]::GetFileNameWithoutExtension($brand.Path)
    $m  = [regex]::Match($bn,'^(?<idx>\d{3})')
    if ($m.Success) {
        $brandIdx = $m.Groups['idx'].Value
        $rowForIdx = $rows | Where-Object {
            $_.clip -and ([regex]::Match($_.clip,'^(?<idx>\d{3})')).Groups['idx'].Value -eq $brandIdx
