...
#    still mark the brand as Matched so it doesnÃ¢â‚¬â„¢t appear in the branded-only evidence list.
...
# Also treat branded files with neither a yyyy-mm-dd date nor a 3-digit index prefix as matched
[CmdletBinding()]
param(
  [Parameter(Position=0)] [string]$RepoRoot      = ".",
  [Parameter(Position=1)] [string]$CinematicRoot = ".\out\autoframe_work\cinematic",
...
_noDateNoIdx = $brandedInfo | Where-Object {
  $leaf = [IO.Path]::GetFileNameWithoutExtension([CmdletBinding()]
param(
  [Parameter(Position=0)] [string]$RepoRoot      = ".",
  [Parameter(Position=1)] [string]$CinematicRoot = ".\out\autoframe_work\cinematic",
  [Parameter(Position=2)] [string]$BrandedRoot   = ".\out\portrait_reels\branded",
...
.Path)
  -not ($leaf -match '^\d{3}__') -and -not ($leaf -match '\d{4}-\d{2}-\d{2}')
}
foreach ($g in [CmdletBinding()]
param(
  [Parameter(Position=0)] [string]$RepoRoot      = ".",
  [Parameter(Position=1)] [string]$CinematicRoot = ".\out\autoframe_work\cinematic",
  [Parameter(Position=2)] [string]$BrandedRoot   = ".\out\portrait_reels\branded",
...
_noDateNoIdx) {
  if (-not $g.Matched) { $g.Matched = $true; $rescued++ }
}
# -- end index-rescue --
$brandedOnlyRows = @()
foreach ($brand in $brandedInfo) {
