...
foreach ($r in $rows) {
  # common fields weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ve seen: clip (folder name), clip_id, index
  __TryAdd-IdxFromValue ([string]$r.clip)    $r
  __TryAdd-IdxFromValue ([string]$r.clip_id) $r
  __TryAdd-IdxFromValue ([string]$r.index)   $r
}

# If we still have branded candidates that wonÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t match rows by idx, try folder names under cinematic.
  ...
  if (Test-Path -LiteralPath $cinRoot) {
    foreach ($d in (Get-ChildItem -LiteralPath $cinRoot -Directory -ErrorAction SilentlyContinue)) {
      ...
      if ($m.Success) {
        ...
        # if rows didnÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t expose this idx yet, create a dummy row object we can attach branded_matches to
