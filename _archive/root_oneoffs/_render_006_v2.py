"""Re-render NEOFC clip 006 v2 - reads camera_x_pct (FIXED bug from v1)."""
import time, os, sys, csv, json, subprocess, traceback, tempfile, shutil
import numpy as np
from pathlib import Path

os.chdir(r"D:\Projects\soccer-video")
RESULT = r"D:\Projects\soccer-video\_tmp\render_006_v2_result.txt"
...
try:
    with open(RESULT, "w") as f:
        f.write(f"Re-render clip {clip_num} v2 (camera_x_pct fix)\n")
    ...
    # Read CSV - FIXED: read camera_x_pct, not ball_x_pct
    ...
    for path in [ball_jsonl, yolo_jsonl, tracker_jsonl]:
        if path.exists():
            bak = path.with_suffix(path.suffix + ".manualv1bak")
    ...
    log("Telemetry written from camera_x_pct")
    t_total = time.time()
