"""Re-render NEOFC clip 006 v3 - tight camera following.
Reads camera_x_pct, patches post_smooth_sigma=0 to prevent Gaussian blur,
uses high smoothing + speed_limit so camera follows anchors directly.
"""
import time, os, sys, csv, json, subprocess, traceback, tempfile, shutil, re
import numpy as np
from pathlib import Path

os.chdir(r"D:\Projects\soccer-video")
RESULT = r"D:\Projects\soccer-video\_tmp\render_010_pipeline_result.txt"
...
try:
    with open(RESULT, "w") as f:
        f.write(f"Re-render clip {clip_num} v3 (tight follow)\n")
    ...
    # Temporarily patch post_smooth_sigma in render script
    render_script = Path("tools/render_follow_unified.py")
    render_text = render_script.read_text(encoding="utf-8")
    render_backup = render_script.with_suffix(".py.smoothbak")
    render_backup.write_text(render_text, encoding="utf-8")
    
    # Replace post_smooth_sigma: 18.0 with 0.0 in cinematic preset
    patched = render_text.replace(
        '"post_smooth_sigma": 18.0',
        '"post_smooth_sigma": 0.0'
    )
    render_script.write_text(patched, encoding="utf-8")
    log("Patched post_smooth_sigma=0 in cinematic preset")

    t_total = time.time()

    # Step 1: Portrait render with aggressive following
    log("Step 1: Portrait render (tight follow)...")
    ...
    # Restore original render script immediately
    render_script.write_text(render_backup.read_text(encoding="utf-8"), encoding="utf-8")
    render_backup.unlink()
    log("Restored original render script")
    ...
    # Restore render script if patched
    render_script = Path("tools/render_follow_unified.py")
    render_backup = render_script.with_suffix(".py.smoothbak")
    if render_backup.exists():
        render_script.write_text(render_backup.read_text(encoding="utf-8"), encoding="utf-8")
        render_backup.unlink()
    with open(RESULT, "a") as f:
