"""
Optical-flow ball trajectory builder v2.
Fixed: uses exact clip name to avoid matching wrong telemetry files.
Uses cx/cy keys consistently.
"""
import cv2
import json
import os
...
TRACKER_PATH = f'out/telemetry/{CLIP}.tracker_ball.jsonl'
...
# YOLO (one per frame, already deduplicated in this file)
yolo = {}
...
# Build anchor list: YOLO validated by tracker where possible,
# plus tracker-only frames
# Known false: f332 (shadow), conf<=0.20 are suspect
FALSE_YOLO = {332}  # confirmed false detections

anchors = {}  # frame -> (cx, cy, source, conf)
for f, d in sorted(yolo.items()):
    if f in FALSE_YOLO:
        print(f"  Skipping false YOLO f{f}")
        continue
    cx, cy, conf = d['cx'], d['cy'], d['conf']
    # Validate against tracker if available
    if f in tracker:
        ...
        if dist < 100:
            ...
            # Disagreement - trust tracker (CSRT tracks actual pixels)
...
t0 = time.time()
# frame -> [(x, y, conf, dist_from_anchor, anchor_frame), ...]
all_tracks = {}
...
# ── Step 7: Save ──
ball_path = os.path.join(OUT_DIR, 'optflow_ball_path_v2.jsonl')
