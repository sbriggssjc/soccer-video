...
versions = [
    ...
    ("v17", "002__manual_v17.diag.csv"),
]

print(f"{'Ver':<5} {'Mean':>6} {'Max':>6} {'P95':>6} {'Rev':>4} {'>30':>5} {'>15':>5} {'YOLO':>5} {'Trkr':>5} {'Cent':>5} {'Hold':>5}")
print("-" * 70)
...
for label, fname in versions:
    ...
    for i in range(1, len(cam_cxs)):
        ...
        if d != 0:
            prev_dir = d
    print(f"{label:<5} {deltas.mean():>6.1f} {deltas.max():>6.1f} {np.percentile(deltas, 95):>6.1f} {reversals:>4} {(deltas>30).sum():>5} {(deltas>15).sum():>5} {sources.get('yolo',0):>5} {sources.get('tracker',0):>5} {sources.get('centroid',0):>5} {sources.get('hold',0):>5}")
