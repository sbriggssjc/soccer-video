...
function Slug([string]$s){
  ...
  # keep letters, numbers, dash/underscore; collapse spaces to underscores
  $t = $s -replace '\s+','_'
...
foreach ($r in $rows) {
  ...
  # build destination folder/file
  $folder = Join-Path $AtomicRoot ("{0}__{1}_vs_{2}" -f $date,$homeTeamName,$awayTeamName)
  $fname  = ("{0}__{1}__{2}_vs_{3}__{4}__t{5:F2}-t{6:F2}.mp4" -f $idx3,$date,$homeTeamName,$awayTeamName,$label,($tS ?? 0),($tE ?? 0)) -replace '\.00\b',''
  ...
  if ($existsForIndex -and -not $ForceRecut) {
    ...
    $plan.Add([pscustomobject]@{
      index=$idxStr; date=$date; status='HAVE'; atomic=$null; master=$null
    }) | Out-Null
    continue
  }
  ...
  if ($null -eq $master) {
    $miss++
    $plan.Add([pscustomobject]@{
      index=$idxStr; date=$date; status='MISSING: no master'; atomic=$outPath; master=$null
    }) | Out-Null
    continue
  }
  ...
  $miss++
  $plan.Add([pscustomobject]@{
    index=$idxStr; date=$date; status=(if($ForceRecut){'RECUT'}else{'MISSING'}); atomic=$outPath; master=$master
  }) | Out-Null
  ...
  if ($DoCut) {
    ...
    if (($tS -ne $null) -and ($tE -ne $null) -and ($tE -gt $tS)) {
      ...
      # safer to re-encode (keyframe independent)
      $ff = "ffmpeg"
      $args = @(
      ...
      # times invalid – just skip cutting but record plan
    }
  }
}
