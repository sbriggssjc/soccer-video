...
with open(ball_csv,"r",newline="") as f:
    rd=csv.DictReader(f)
    for r in rd:
        N.append(int(r["n"])); bx.append(float(r["cx"])); by.append(float(r["cy"])); conf.append(float(r["conf"]))
...
def score_and_zoom(cx, cy, base_mx, base_my, safety, zcap):
    ...
    z_need = np.maximum.reduce([np.ones_like(dx_edge), z_need_edge_x, z_need_edge_y, z_need_ball])
    def ema_vec(a,alpha):
        o=a.copy()
...
TARGET_P95 = 6.0; TARGET_MAE = 10.0; ZCAP = 3.4
MAX_ITERS  = 7
                ...
                for safety in SAF_rng:
                    ...
                    if np.any(panic):
                        ...
                        for i in range(1,len(cx2)):
                            ...
                            cx2[i] = cx2[i-1] + sx; cy2[i] = cy2[i-1] + sy
                        cx, cy = cx2, cy2
