...
if USE_YOLO:
    ...
    for result in model.predict(source=in_path, conf=conf_min, stream=True, imgsz=imgsz, verbose=False):
        ...
        if result.boxes is not None and len(result.boxes) > 0:
            b = result.boxes; i = int(b.conf.argmax().item())
...
while True:
    ...
    if best is None: cx, cy, conf = last[0], last[1], 0.0
    else: cx, cy, conf = best[0], best[1], 0.5; last = (cx, cy)
