...
def zoom_for_subject(W,H,cx,cy,w,h,zmin,zmax,margin):
    ...
    z_req = min(z1, z2)
    # keep some room from edges
    half_h_edge = max(12.0, min(cy, H-cy))
    half_w_edge = max(12.0, min(cx, W-cx))
...
def first_person_autopick(frame):
    H,W=frame.shape[:2]
    try:
        from ultralytics import YOLO
        yolo=YOLO("yolov8m.pt")
        preds=yolo.predict(frame, classes=[0], conf=0.20, iou=0.45, verbose=False)
        if preds and preds[0].boxes is not None and len(preds[0].boxes.xyxy)>0:
            ...
            for (x1,y1,x2,y2), score in zip(preds[0].boxes.xyxy.cpu().numpy(),
                ...
                s = float(score)*(1.0 + area/(W*H*0.12)) * (1.0/(1.0+d/120.0))
                if s>best_score:
                    best_score=s; best=(float(x1),float(y1),float(w),float(h))
            if best: return best
    except Exception:
        pass
    # fallback to center box
    w=H*0.18; h=H*0.36; return (W/2-w/2, H/2-h/2, w, h)

def track_with_redetect(src, redetect_every=6, conf=0.20, iou_keep=0.20, debug_mp4=None):
    ...
    init_box = first_person_autopick(frame)

    tracker=make_tracker(); tracker.init(frame, init_box)
    ...
    for n in range(N):
        ...
        if ok:
            ...
            if n>3:
                ...
                if dx<0.35 and dy<0.35: stagnant+=1
                else: stagnant=0
            pad=10
            if (x<pad or y<pad or x+w>W-pad or y+h>H-pad) or stagnant>=5:
                need_redetect=True

        if need_redetect and yolo is not None:
            preds=yolo.predict(frame, classes=[0], conf=conf, iou=0.45, verbose=False)
            best=None; best_score=-1.0
            guess = box if ok else last
            gcx=(guess[0]+guess[2]*0.5); gcy=(guess[1]+guess[3]*0.5)
            for (x1,y1,x2,y2), score in zip(getattr(preds[0].boxes,"xyxy",[]).cpu().numpy() if preds else [],
                                            getattr(preds[0].boxes,"conf",[]).cpu().numpy() if preds else []):
                b=(float(x1),float(y1),float(x2-x1),float(y2-y1))
                cxx=(b[0]+b[2]*0.5); cyy=(b[1]+b[3]*0.5)
                d=((cxx-gcx)**2+(cyy-gcy)**2)**0.5
                s=float(score) * (1.0/(1.0+d/80.0))
                if iou(b,guess)>iou_keep: s*=1.35
...
def plan_path(W,H,N, cx,cy,bw,bh, lookahead=14, vmax=48.0, amax=7.5, edge_margin=0.38, zmin=1.00, zmax=1.26):
    ...
    z_list=[]
    for i in range(N):
        sp = 0.0 if i==0 else min(1.0, math.hypot(cx_s[i]-cx_s[i-1], cy_s[i]-cy_s[i-1])/16.0)
        m = edge_margin*(1.0 + 0.45*sp)
        z = zoom_for_subject(W,H, cx_s[i],cy_s[i], bw[i],bh[i], zmin,zmax, m)
        z_list.append(z)
    z=np.array(z_list)
    for i in range(1,N): z[i]=0.30*z[i]+0.70*z[i-1]
...
def main():
    ...
    ap.add_argument("--amax", type=float, default=7.5)
    ap.add_argument("--edge_margin", type=float, default=0.38)
    ap.add_argument("--z_min", type=float, default=1.00)
    ap.add_argument("--z_max", type=float, default=1.26)
    args=ap.parse_args()

    try:
        W,H,N, cx,cy,bw,bh = track_with_redetect(
            args.src, redetect_every=6, conf=0.20, iou_keep=0.20, debug_mp4=args.debug_mp4
        )
        if N==0:
        ...
        if np.std(cx)+np.std(cy) < 1.0:
            ...
            log("[WARN] Tracking looks flat; wrote center vars. Check debug MP4.")
            return
        ...
        traceback.print_exc()
        # Never leave PowerShell without vars; write safe fallback
        try:
            write_center_vars(args.vars_out, 1080, 1920)
            log("[FAIL] Exception; wrote center vars. See console for traceback.")
        except Exception:
