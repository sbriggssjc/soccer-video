param(
  ...
  # tune these to reduce choppiness
  [double]$ActionMergeGap = 1.0,   # merge action slices if <= this gap
  [double]$MinSpanSec     = 1.6,   # drop clips shorter than this
...
function Get-VideoDurationSec([string]$path) {
  $durStr = & ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 $path 2>$null
  [double]$d = 0; TryParse-Double $durStr ([ref]$d) | Out-Null
  if ($d -le 0) { throw "Could not read duration via ffprobe for $path" }
...
function Read-Spans([string]$csvPath, [double]$padBefore=0.0, [double]$padAfter=0.0, [string]$source="") {
  ...
  foreach ($r in $rows) {
    ...
    if (TryParse-Double $r.start ([ref]$a) -and TryParse-Double $r.end ([ref]$b) -and $b -gt $a) {
      ...
      $t1 = $b + $padAfter
      $sp += [pscustomobject]@{ t0=$t0; t1=$t1; src=$source; score=0.0 }
    }
  }
  return $sp
}
...
function Merge-Spans($spans, [double]$minGap=0.0) {
  ...
  foreach ($s in $ordered) {
    ...
    if ($s.t0 -le $last.t1 + $minGap) {
      # merge; keep widest, preserve best score and concatenate sources
      if ($s.t1 -gt $last.t1) { $last.t1 = $s.t1 }
      if ($s.PSObject.Properties.Match('score')) {
        if ($last.score -lt $s.score) { $last.score = $s.score }
      }
      if ($s.PSObject.Properties.Match('src')) {
        if ($last.src -notlike "*$($s.src)*") { $last.src = "$($last.src)+$($s.src)" }
      }
    } else {
      $out += $s
    }
...
if ($final.Count -eq 0) { Write-Warning "No spans selected. Check your CSVs."; exit 0 }

# build filter_complex into a file (avoid long cmdline)
...
# single encode = smooth cuts, no DTS issues, short cmdline
