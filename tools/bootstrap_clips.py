...
# ---------------------------------------------------------------------------
# Timestamp parsing  (MM:SS, MM:SS:cs, and plain seconds with commas)
# ---------------------------------------------------------------------------

_MMSS_CS_RE = re.compile(r"^(\d+):(\d+):(\d+(?:\.\d+)?)$")  # MM:SS:cs
...
def parse_timestamp(raw: str) -> Optional[float]:
    ...
    # MM:SS:centiseconds  (e.g., "5:15:00" = 5m 15.00s = 315s)
    m = _MMSS_CS_RE.match(raw)
    if m:
        minutes = float(m.group(1))
        seconds = float(m.group(2))
        centis = float(m.group(3))
        return minutes * 60 + seconds + centis / 100

    # MM:SS or MM:SS.f  (e.g., "6:27" = 6m 27s = 387s)
...
def load_plays_manual(path: Path) -> list[ClipDef]:
    ...
    with path.open("r", newline="", encoding="utf-8-sig") as fh:
        ...
        for row in reader:
            ...
            t0 = parse_timestamp(row.get("master_start", ""))
            t1 = parse_timestamp(row.get("master_end", ""))
...
def load_clip_index(path: Path) -> list[ClipDef]:
    ...
    with path.open("r", newline="", encoding="utf-8-sig") as fh:
        ...
        for row in reader:
            ...
            t0 = parse_timestamp(row.get("start", ""))
            t1 = parse_timestamp(row.get("end", ""))
            if t0 is None or t1 is None or t1 <= t0:
                continue
            clips.append(ClipDef(clip_num=num, label=label, t_start_s=t0, t_end_s=t1))
    return clips
