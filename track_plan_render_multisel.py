...
def main():
    ...
    ap.add_argument("--sel",   dest="sel_json", required=True)  # anchors
    ...
    ap.add_argument("--vmax", type=float, default=84.0)
    ap.add_argument("--amax", type=float, default=16.0)
    ap.add_argument("--edge_margin", type=float, default=0.34)
    ap.add_argument("--z_min", type=float, default=1.00)
    ap.add_argument("--z_max", type=float, default=1.16)
    ap.add_argument("--reacq_every", type=int, default=6)       # YOLO global search cadence
    ap.add_argument("--bad_cost", type=float, default=0.42)     # above this, force re-acquire
    args = ap.parse_args()

    sel = json.load(open(args.sel_json,"r",encoding="utf-8"))
    anchors = sel.get("anchors", [])
    if not anchors: raise SystemExit("No anchors in selection JSON.")
    ...
    def set_anchor(frame_idx):
        ...
        cur_anchor = anchors[a_idx]
        if a_idx+1 < len(anchors): next_anchor = anchors[a_idx+1]
        else: next_anchor = None
        # jump to anchor frame, seed box & histogram
    ...
    # init at first anchor
    cur_box=(0,0,32,64); target_hist=None; pts=None
    ok = set_anchor(anchors[0]["frame"])
    if not ok: raise SystemExit("Failed to initialize at first anchor.")

    # write full-res debug with box
    ...
    for n in range(N):
        ...
        # hop to next anchor when we reach/past it
        ...
        # do global re-acquire either periodically or when the propagated candidate looks bad
        do_reacq = (n % args.reacq_every)==0
        # quick cost estimate to decide forced re-acq
        ...
        best, cost = choose_candidate(cands, cur_box, target_hist)
        if best is None:
            best = prop_box
        ...
        widths.append(float(max(8.0,w)))
        heights.append(float(max(16.0,h)))
        ns.append(float(n))

        vis = frame.copy()
        cv2.rectangle(vis,(int(x),int(y)),(int(x+w),int(y+h)),(0,255,0),2)
        cv2.circle(vis,(int(cx),int(cy)),4,(0,255,0),-1)
        cv2.putText(vis,f"n={n} cost={cost:.2f}",(12,28),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,0),2)
        dbg.write(vis)
    ...
    # path planning (fast but smooth follow)
